...
class Aggregator:
    ...
    def aggregate_model_params(self, model_params_from_nodes_db_links):
        ...
        all_node_updates = node_ref.get()

        decoded_params = []
        for key, value in all_node_updates.items():
            model_weights = value.get('model_update')
            if not model_weights:
                raise ValueError(f"Missing model_weights in entry with key: {key}")
